...
def page_digital_twin():
    ...
    # Fetch infrastructure counts - using correct column names for each table
    infra_counts = run_query("""
        SELECT 
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.SITES WHERE STATUS = 'ACTIVE') as SITES,
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.TOWERS WHERE LIFECYCLE_STATUS = 'OPERATIONAL') as TOWERS,
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.ANTENNAS WHERE LIFECYCLE_STATUS = 'OPERATIONAL') as ANTENNAS,
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.DATA_CENTERS WHERE LIFECYCLE_STATUS != 'DECOMMISSIONED') as DATA_CENTERS,
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.BROADCAST_TRANSMITTERS WHERE STATUS = 'ACTIVE') as TRANSMITTERS,
            (SELECT COUNT(*) FROM TDF_DATA_PLATFORM.INFRASTRUCTURE.FIBRE_NETWORK WHERE STATUS = 'ACTIVE') as FIBRE_SEGMENTS
    """)
