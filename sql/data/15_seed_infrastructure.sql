...
SELECT 
    'DC-REG-' || ROW_NUMBER() OVER () AS DC_ID,
    SITE_ID,
...
SELECT 
    'DC-EDGE-' || LPAD(ROW_NUMBER() OVER (), 3, '0') AS DC_ID,
    SITE_ID,
    'DC-EDGE-' || LPAD(ROW_NUMBER() OVER (), 3, '0') AS DC_CODE,
    'TDF Edge DC ' || ROW_NUMBER() OVER () AS DC_NAME,
    'EDGE' AS DC_TYPE,
    'TIER2' AS DC_TIER,
...
SELECT 
    'INST-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS INSTALLATION_ID,
    s.SITE_ID,
    CASE MOD(ROW_NUMBER() OVER (), 4)
        ...
        ELSE 'OP-FREE'
    END AS OPERATOR_ID,
    'CTR-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS CONTRACT_NUMBER,
...
SELECT 
    'EQP-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS EQUIPMENT_ID,
    'EQP-' || et.EQUIPMENT_TYPE_CODE || '-' || LPAD(ROW_NUMBER() OVER (), 5, '0') AS EQUIPMENT_CODE,
    s.SITE_ID,
    t.TOWER_ID,
