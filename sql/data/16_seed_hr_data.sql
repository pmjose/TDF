...
SELECT 
    'EMP-' || LPAD(ROW_NUMBER() OVER (ORDER BY 1), 5, '0') AS EMPLOYEE_ID,
    'TDF' || LPAD(ROW_NUMBER() OVER (ORDER BY 1), 5, '0') AS EMPLOYEE_CODE,
    CASE MOD(ROW_NUMBER() OVER (ORDER BY 1), 20)
    ...
    END AS FIRST_NAME,
    CASE MOD(ROW_NUMBER() OVER (ORDER BY 1), 25)
        ...
        ELSE 'Dupont'
    END AS LAST_NAME,
    'employee' || ROW_NUMBER() OVER (ORDER BY 1) || '@tdf.fr' AS EMAIL,
    ...
    CASE MOD(ROW_NUMBER() OVER (ORDER BY 1), 15)
    ...
    CASE 
        ...
        ELSE 'DIRECTOR'
    END AS JOB_LEVEL,
    CASE MOD(ROW_NUMBER() OVER (ORDER BY 1), 13)
    ...
    CASE MOD(ROW_NUMBER() OVER (ORDER BY 1), 13)
        ...
        ELSE 'REG-IDF'
    END AS REGION_ID,
    CASE WHEN MOD(ROW_NUMBER() OVER (ORDER BY 1), 3) < 2 THEN TRUE ELSE FALSE END AS IS_FIELD_WORKER,
    'BAND_' || (MOD(ROW_NUMBER() OVER (ORDER BY 1), 8) + 2) AS SALARY_BAND,
...
SELECT 
    'SM-' || LPAD(ROW_NUMBER() OVER (ORDER BY 1), 7, '0') AS SKILL_MATRIX_ID,
    e.EMPLOYEE_ID,
    s.SKILL_ID,
...
SELECT 
    'CAP-' || LPAD(ROW_NUMBER() OVER (ORDER BY 1), 6, '0') AS CAPACITY_ID,
...
SELECT 
    'DIV-' || LPAD(ROW_NUMBER() OVER (ORDER BY 1), 5, '0') AS METRIC_ID,
    cal.DATE_KEY AS YEAR_MONTH,
