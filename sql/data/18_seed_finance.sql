...
SELECT 
    'REV-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS REVENUE_ID,
    2025 AS FISCAL_YEAR,
...
SELECT 
    'REVC-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS REVENUE_CLIENT_ID,
    2025 AS FISCAL_YEAR,
    MONTH(cal.DATE_KEY) AS FISCAL_MONTH,
    cal.DATE_KEY AS PERIOD_DATE,
    op.OPERATOR_ID,
...
SELECT 
    'CAPX-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS ACTUAL_ID,
    2025 AS FISCAL_YEAR,
    MONTH(cal.DATE_KEY) AS FISCAL_MONTH,
    cal.DATE_KEY AS PERIOD_DATE,
    cb.CAPEX_CATEGORY,
...
SELECT 
    'EBITDA-' || LPAD(ROW_NUMBER() OVER (), 5, '0') AS METRIC_ID,
    2025 AS FISCAL_YEAR,
    MONTH(cal.DATE_KEY) AS FISCAL_MONTH,
    cal.DATE_KEY AS PERIOD_DATE,
...
SELECT 
    'AE-' || LPAD(ROW_NUMBER() OVER (), 8, '0') AS ENTRY_ID,
    'JNL-2025-' || LPAD(FLOOR(ROW_NUMBER() OVER () / 2), 6, '0') AS JOURNAL_ID,
    MOD(ROW_NUMBER() OVER (), 2) + 1 AS LINE_NUMBER,
    ...
    CASE MOD(ROW_NUMBER() OVER (), 10)
        ...
        ELSE 'LIABILITY'
    END AS ACCOUNT_TYPE,
    CASE WHEN MOD(ROW_NUMBER() OVER (), 2) = 0 THEN UNIFORM(1000, 500000, RANDOM()) ELSE 0 END AS DEBIT_EUR,
    CASE WHEN MOD(ROW_NUMBER() OVER (), 2) = 1 THEN UNIFORM(1000, 500000, RANDOM()) ELSE 0 END AS CREDIT_EUR,
    'CC-HOST' AS COST_CENTER_ID,
    'BU-HOST' AS BU_ID,
    'Accounting entry' AS DESCRIPTION,
    CASE MOD(ROW_NUMBER() OVER (), 3) WHEN 0 THEN 'INVOICE' WHEN 1 THEN 'PAYMENT' ELSE 'JOURNAL' END AS DOCUMENT_TYPE,
    'DOC-2025-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS DOCUMENT_NUMBER,
    'TDF_ERP' AS SOURCE_SYSTEM,
    CASE MOD(ROW_NUMBER() OVER (), 3) WHEN 0 THEN 'AP' WHEN 1 THEN 'AR' ELSE 'GL' END AS SOURCE_MODULE,
    'SRC-' || LPAD(ROW_NUMBER() OVER (), 8, '0') AS SOURCE_RECORD_ID,
    'finance_user' AS CREATED_BY,
    TRUE AS IS_POSTED,
    CASE WHEN MOD(ROW_NUMBER() OVER (), 10) IN (2, 3) THEN TRUE ELSE FALSE END AS IS_ESG_RELEVANT
...
SELECT 
    'RENEW-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS FORECAST_ID,
    e.EQUIPMENT_ID,
    e.SITE_ID,
