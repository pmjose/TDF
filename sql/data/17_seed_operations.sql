...
SELECT 
    'WO-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS WORK_ORDER_ID,
    'WO-2025-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS WORK_ORDER_NUMBER,
...
SELECT 
    'MAINT-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS MAINTENANCE_ID,
    wo.WORK_ORDER_ID,
    e.EQUIPMENT_ID,
    e.SITE_ID,
    CASE MOD(ROW_NUMBER() OVER (), 4)
...
SELECT 
    'EQST-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS STATUS_ID,
    EQUIPMENT_ID,
...
SELECT 
    'ALLOC-' || LPAD(ROW_NUMBER() OVER (), 7, '0') AS ALLOCATION_ID,
    e.EMPLOYEE_ID,
    wo.WORK_ORDER_ID,
...
SELECT 
    'INC-' || LPAD(ROW_NUMBER() OVER (), 6, '0') AS INCIDENT_ID,
    'INC-2025-' || LPAD(ROW_NUMBER() OVER (), 5, '0') AS INCIDENT_NUMBER,
    CASE MOD(ROW_NUMBER() OVER (), 4)
        WHEN 0 THEN 'OUTAGE'
