...
CREATE OR REPLACE TABLE ACCOUNTING_ENTRIES (
    ...
    NET_AMOUNT_EUR FLOAT GENERATED ALWAYS AS (DEBIT_EUR - CREDIT_EUR) VIRTUAL,
...
CREATE OR REPLACE INDEX IDX_REVENUE_PERIOD ON REVENUE_BY_SEGMENT(PERIOD_DATE);
CREATE OR REPLACE INDEX IDX_REVENUE_SEGMENT ON REVENUE_BY_SEGMENT(SEGMENT_LEVEL1);
CREATE OR REPLACE INDEX IDX_REVENUE_CLIENT_OP ON REVENUE_BY_CLIENT(OPERATOR_ID);
CREATE OR REPLACE INDEX IDX_CAPEX_YEAR ON CAPEX_BUDGET(FISCAL_YEAR);
CREATE OR REPLACE INDEX IDX_CAPEX_ACTUAL_PERIOD ON CAPEX_ACTUALS(PERIOD_DATE);
CREATE OR REPLACE INDEX IDX_ACCOUNTING_DATE ON ACCOUNTING_ENTRIES(POSTING_DATE);
CREATE OR REPLACE INDEX IDX_ACCOUNTING_SOURCE ON ACCOUNTING_ENTRIES(SOURCE_SYSTEM, SOURCE_RECORD_ID);
CREATE OR REPLACE INDEX IDX_RENEWAL_YEAR ON RENEWAL_FORECAST(REPLACEMENT_YEAR);
