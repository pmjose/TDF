...
CREATE OR REPLACE SEMANTIC VIEW TDF_DATA_PLATFORM.ANALYTICS.SV_RESOURCE_CAPACITY
    ...
    METRICS (
        CAP.TOTAL_HEADCOUNT AS SUM(CAP.HEADCOUNT_F) COMMENT='Total headcount',
        CAP.TOTAL_FTE AS SUM(CAP.FTE_AVAILABLE_F) COMMENT='Total FTE available',
        CAP.TOTAL_DEMAND AS SUM(CAP.DEMAND_FTE_F) COMMENT='Total demand FTE',
        CAP.TOTAL_GAP AS SUM(CAP.FTE_GAP_F) COMMENT='Total FTE gap',
        CAP.AVG_UTILIZATION AS AVG(CAP.UTILIZATION_F) COMMENT='Average utilization'
    )
...
CREATE OR REPLACE SEMANTIC VIEW TDF_DATA_PLATFORM.ANALYTICS.SV_ESG_REPORTING
    ...
    METRICS (
        ESG.TOTAL_EMISSIONS AS SUM(ESG.CARBON_TONNES_F) COMMENT='Total carbon emissions',
        ESG.AVG_EGALITE AS AVG(ESG.EGALITE_INDEX_F) COMMENT='Average Index Égalité'
    )
...
CREATE OR REPLACE SEMANTIC VIEW TDF_DATA_PLATFORM.ANALYTICS.SV_DIGITAL_TWIN
    ...
    METRICS (
        INFRA.TOTAL_SITES AS SUM(INFRA.SITE_COUNT_F) COMMENT='Total sites',
        INFRA.TOTAL_DISCREPANCIES AS SUM(INFRA.DISCREPANCY_COUNT_F) COMMENT='Total discrepancies'
    )
...
CREATE OR REPLACE SEMANTIC VIEW TDF_DATA_PLATFORM.ANALYTICS.SV_CAPEX_LIFECYCLE
    ...
    METRICS (
        EQUIP.TOTAL_EQUIPMENT AS SUM(EQUIP.EQUIPMENT_COUNT_F) COMMENT='Total equipment',
        EQUIP.TOTAL_COST AS SUM(EQUIP.REPLACEMENT_COST_F) COMMENT='Total replacement cost'
    )
